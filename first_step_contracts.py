...
from shared_functions import extract_dates_re, phrase_position, header_info, read_filing, search_phrases, search_phrases_with_context
...
phrases_first_step = ["CREDIT AGREEMENT", "LOAN AGREEMENT", "CREDIT FACILITY", "LOAN AND SECURITY AGREEMENT", "LOAN & SECURITY AGREEMENT",
           "REVOLVING CREDIT", "FINANCING AND SECURITY AGREEMENT", "FINANCING & SECURITY AGREEMENT", "CREDIT AND GUARANTEE AGREEMENT",
           "CREDIT & GUARANTEE AGREEMENT", "NOTE AGREEMENT"]
...
# Using the screening_resuts_ CSV file saved by "screening_of_filings_for_phrases.py", build a dictionary of filings to parse.
# The dictionary keys are the file paths and the values are lists of SEQUENCE_tag values (so only the relevant attachments are parsed).

def dict_of_documents_with_phrases(metadata_file_path, phrases_first_step):
    ...
    # Check the "Search results" column for the phrases in the phrases_first_step list. Only keep rows where at least one of the phrases is found
    metadata_df = metadata_df[metadata_df["Search results"].apply(lambda x: any(phrase in x for phrase in phrases_first_step))]
    # Create a dictionary from filename and SEQUENCE_tag columns. The keys should be the filename (including path) and the values should be a list of relevant SEQUENCE_tag values
...
dict_of_filings_to_parse = dict_of_documents_with_phrases(metadata_file_path, phrases_first_step)
...
# Function to find the distance (in number of words) between a phrase and the first "TABLE OF CONTENTS" phrase
def phrase_toc_distance(extracted_text, search_results, tocs):
    # Split and join the text to remove extra white spaces that are not line breaks:
    extracted_text = ' '.join(extracted_text.split())

    phrase_toc_distance_result = {
        'distance_start_firstTOC': float('inf'),
        'distance_phrase_firstTOC': float('inf'),
        'first_TOC_phrase': None
    }

    if search_results:
        toc_result = search_phrases(extracted_text, tocs)
        if toc_result:
            first_toc_pos = float('inf')
            for toc in toc_result:
                toc_positions = phrase_position(extracted_text, toc)
                if toc_positions:
                    try:
                        if min(toc_positions) < first_toc_pos:
                            first_toc_pos = min(toc_positions)
                            phrase_toc_distance_result['first_TOC_phrase'] = toc
                            phrase_toc_distance_result['distance_start_firstTOC'] = len(extracted_text[:first_toc_pos].split())
                    except ValueError as e:
                        phrase_toc_distance_result['error'] = f"Error finding minimum TOC position: {str(e)}"
                        return phrase_toc_distance_result

            # Filter phrases that appear before the first TOC position
            valid_phrase_positions = [pos for phrase in search_results for pos in phrase_position(extracted_text, phrase) if pos < first_toc_pos]

            if valid_phrase_positions:
                try:
                    # Find the phrase position that is closest but comes before the first TOC position
                    closest_phrase_pos = max(valid_phrase_positions)
                except ValueError as e:
                    phrase_toc_distance_result['error'] = f"Error finding maximum valid phrase position: {str(e)}"
                    return phrase_toc_distance_result

                try:
                    # Calculate the text between the closest phrase and the first TOC position
                    text_between = extracted_text[closest_phrase_pos:first_toc_pos]
                    word_count = len(text_between.split())
                except Exception as e:
                    phrase_toc_distance_result['error'] = f"Error slicing text or counting words: {str(e)}"
                    return phrase_toc_distance_result

                if word_count <= 300:
                    try:
                        phrase_toc_distance_result.update(extract_dates_re(extracted_text))
                        phrase_toc_distance_result["num_dates"] = len(phrase_toc_distance_result["MDY_dates"])
                    except Exception as e:
                        phrase_toc_distance_result['error'] = f"Error updating result with dates: {str(e)}"
                        return phrase_toc_distance_result
                    try:
                        phrase_toc_distance_result["SFAS_phrase_ddl"] = search_phrases_with_context(extracted_text, sfas_phrases_ddl)
                        phrase_toc_distance_result["SFAS_phrase_new"] = search_phrases_with_context(extracted_text, sfas_phrases_new)
                    except Exception as e:
                        phrase_toc_distance_result['error'] = f"Error updating result with SFAS phrases: {str(e)}"
                        return phrase_toc_distance_result

                try:
                    # Update the result dictionary with the calculated values
                    phrase_toc_distance_result['distance_phrase_firstTOC'] = word_count
                    phrase_toc_distance_result['amended'] = search_phrases(extracted_text[:first_toc_pos].lower(), ["amended", "restated"])
                except Exception as e:
                    phrase_toc_distance_result['error'] = f"Error updating final results: {str(e)}"
                    return phrase_toc_distance_result
    ...
    try:
        ...
        if include_document(document, result, excluded_types, excluded_texts, excluded_descriptions, SEQUENCE_tag_values, document_header_tags):
            ...
            try:
                ...
                return result
            try:
                search_results = search_phrases(' '.join(split_text), phrases_first_step)
                result["Search results"] = search_results
            except Exception as e:
                result["error"] = f"Error searching phrases: {str(e)}"
                return result
            try:
                phrase_toc_distance_dict = phrase_toc_distance(extracted_text, search_results, tocs)
                result.update(phrase_toc_distance_dict)
            except Exception as e:
                result["error"] = f"Error calculating phrase TOC distance: {str(e)}"
                return result
    except Exception as e:
...
def process_file(file_path, max_document_threads, dict_of_filings_to_parse):
    ...
    results = []
    try:
        # Get the last 24 characters of the filename
        #filename = file_path[-24:]
    ...
    try:
        ...
        results = [{"error": f"ThreadPoolExecutor error: {str(e)}", "filename": file_path}]
...
#sleep for 3 seconds
    ...
    combined_csv = os.path.join(results_directory, f"first_step_contracts_{result_form_type}.csv")
